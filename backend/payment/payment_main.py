...
def upgrade_plan_in_storage(upgrade_plan_name, user_id):
    response = patch(
        f"http://localhost:8002/upgrade_plan?user_id={user_id}",
        json={"upgrade_plan_name": upgrade_plan_name},
    )
    if response.status_code == 200:
        print("Żądanie zostało pomyślnie wysłane.")
    else:
        print(
            "Wystąpił problem podczas wysyłania żądania. Kod statusu:",
...
async def payment_success(
    user_id: str, upgrade_plan_name: str, background_tasks: BackgroundTasks
) -> RedirectResponse:
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Payment database returned an error: {str(e)}"
        )
...
async def create_payment(
    data: UpgradePlanArgs, info: Annotated[str, Depends(verify_token)]
) -> RedirectResponse:
    payment_repo = PaymentRepository()
    ...
    try:
        checkout_session = create_checkout_session(
            price_id=price_id,
            user_id=info["sub"],
            upgrade_plan_name=data.upgrade_plan_name,
        )
