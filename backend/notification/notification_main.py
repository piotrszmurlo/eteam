...
async def add_user(
    user_email: UserEmailInput, token: Annotated[str, Depends(verify_token)]
) -> UserIdResponse:
    notification_repo = NotificationRepository()
    user = UserModel(
        user_id=token["sub"],
        user_name=token["given_name"],
...
async def upgrade_plan(
    upgrade_details: UpgradePlan, token: Annotated[str, Depends(verify_token)]
) -> UrlResponseModel:

    print(
        f"NOTIFICATION: Your current plan is {upgrade_details.current_plan_name} and you need to upgrade to {upgrade_details.upgrade_plan_name}"
    )

    create_payment_details = UpgradePlanArgs(
        upgrade_plan_name=upgrade_details.upgrade_plan_name
    )

    url_response = UrlResponseModel(
        url="http://localhost:8003/create_payment", data=create_payment_details
    )
...
async def upgrade_plan_success(
    data: UpgradePlanArgs, token: Annotated[str, Depends(verify_token)]
) -> dict[str, str]:
    return {"message": f"Your plan has been upgraded to{data.upgrade_plan_name}"}


@notification_app.get("/upgrade_plan_fail")
async def upgrade_plan_fail(
