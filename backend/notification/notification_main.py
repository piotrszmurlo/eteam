...
from notification.models import UserIdResponse, UserModel, UserEmailInput, UpgradePlan, CreatePaymentDetails
...
async def upgrade_plan(upgrade_details: UpgradePlan, token: Annotated[str, Depends(verify_token)]):
    ...
    create_payment_details = CreatePaymentDetails(upgrade_plan_name=upgrade_details.upgrade_plan_name)
...
async def upgrade_plan_success(data: CreatePaymentDetails, token: Annotated[str, Depends(verify_token)]):
