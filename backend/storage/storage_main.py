...
async def add_file(
    file_input: UploadFile, token: Annotated[str, Depends(verify_token)]
) -> FileIdModel:
    storage_repo = StorageRepository()
    ...
    file = FileModel(
        user_id=token["sub"], file_name=file_input.filename, file_size=contents_mb_size
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="An unexpected error ocurred when trying to save the file",
        )
    ...
    upgrade_details = UpgradePlan(
        current_plan_name=current_plan_name, upgrade_plan_name=upgrade_plan_name
    )
    detail_message = f"Storage limit exceded. Currently your plan is {current_plan_name}. You lack {required_space} Mb."
    detail_data = UrlResponseModel(
        url="http://localhost:8004/upgrade_plan",
        data=upgrade_details.model_dump(),
    )

    raise HTTPException(
        status_code=413,
        detail={"message": detail_message, "data": detail_data.model_dump()},
    )


@storage_app.get("/files")
async def get_files_for_user(
    info: Annotated[str, Depends(verify_token)]
) -> list[FileModel]:
    storage_repo = StorageRepository()
    user_id = info["sub"]
    try:
...
async def get_file_by_id(
    file_id: str, token: Annotated[str, Depends(verify_token)]
) -> FileResponse:
    storage_repo = StorageRepository()
    user_id = token["sub"]
...
async def delete_file(
    file_id: uuid.UUID, token: Annotated[str, Depends(verify_token)]
) -> FileIdModel:
    storage_repo = StorageRepository()
    file_manager = FileManager(user_id=token["sub"])
    try:
...
async def upgrade_plan(data: UpgradePlanArgs, user_id: str):
    storage_repo = StorageRepository()
    try:
        storage_repo.upgrade_plan(
            user_id=user_id, upgrade_plan_name=data.upgrade_plan_name
        )
    except CannotGetPlan:
    ...
    return {
        "message": f"Plan of user {user_id} was upgraded to {data.upgrade_plan_name} in storage DB."
    }
